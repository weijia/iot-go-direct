...
func (loraModule LoraModule) SendHeartbeatForList(ctx context.Context,
	...
	for _, nodeIdStr := range nodeList {
		...
		reply := loraModule.SendNodeMsgWithRetryOrTimeoutWillBlock(node.GetHeartbeatMsg(nodeIdStr), util.HEARTBEAT_RETRY_CNT, node.HEARTBEAT_REPLY)
		...
		if reply == nil {
			...
			Module0.SendNodeMsgWithRetryOrTimeoutWillBlock(node.GetNodeInitMsg(nodeIdStr, module), util.NODE_INIT_RETRY_CNT, node.CONFIG_NODE_REPLY)
