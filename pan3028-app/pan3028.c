...
extern uint8_t PAN3028_read_reg(uint8_t addr);
extern uint32_t PAN3028_write_reg(uint8_t addr,uint8_t value);
...
{ 
	...
	printf("PAN3028_write_reg read from 0x%x again, val: 0x%x\n", addr, tmpreg);
	if(tmpreg == value)
	{
		return OK;
	}
 ...
 */
extern int spi_readwrite_reg_with_buf(uint8_t reg, void* tx_data, void* rx_data, int len);
/**
...
{ 
	...
	// }
	spi_readwrite_reg_with_buf(addr_w, buffer, NULL, size);
	rf_port.spi_cs_high();	
}
...
{   
	...
	// }
	spi_readwrite_reg_with_buf(addr_w, NULL, buffer, size);
	rf_port.spi_cs_high();	
}
...
{ 	 
	...
	// 	rf_port.spi_readwrite(buffer[i]); 
	// }
	spi_readwrite_reg_with_buf(addr_w, buffer, NULL, len);
	rf_port.spi_cs_high();	
	...
	// }
	spi_readwrite_reg_with_buf(0x00 | (addr<<1), buffer, temreg, len);
	rf_port.spi_cs_high(); 
...
uint32_t PAN3028_deepsleep_wakeup(void)
{
	...
	PAN3028_write_reg(REG_SYS_CTL, rstreg);
	pr_info("before dely\n");
	// rf_port.delayus(5000);
	
	if(PAN3028_write_reg(REG_OP_MODE, PAN3028_MODE_DEEP_SLEEP) != OK)	
	{
		pr_info("Returning fail, Line number is: %d\n",
           __LINE__);
		return FAIL;
	}
	rf_port.delayus(10);
	...
	{
		rf_port.delayus(10);
		pr_info("Returning OK, Line number is: %d\n",
           __LINE__);
		return OK;
	}
}
...
float PAN3028_get_snr(void)
{
	...
	uint32_t sf_val;
	printf("Before read data from reg\n");
	...
	printf("Before float calculation\n");
