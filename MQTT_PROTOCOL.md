...
| `color` | string | 是 | 颜色编码，每 2 字符为一组：第 1 位为区域号（1-8），第 2 位为颜色值（0-F） |
...
| `update_glass_color_request` 中 color 不是合法 hex | 该节点颜色被标记为无效（`SetColorForNodeAsInvalid`），直接加入回复 |
