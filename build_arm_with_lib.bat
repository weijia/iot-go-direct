...
@REM maybe because -X can only change the value for the listed go files, so only var in main package can be changed
go build -a -v  -ldflags="-s -w -X main.Version=%CURRENT_VERSION% -X main.GitCommit=%LATEST_COMMIT% -X main.BuildDate=%CURRENT_DATE_TIME%" ./cmd/cobra_main/main.go ./cmd/cobra_main/main_loop.go ./cmd/cobra_main/version_and_supervisord.go ./cmd/cobra_main/lora_service.go ./cmd/cobra_main/version.go
@REM go build -a -v  -ldflags="-s -w" cmd/iot/main.go
@REM go build -a -v  -ldflags="-s -w" cmd/lora_service/lora_service.go
@REM move main iot_go
sync.bat
